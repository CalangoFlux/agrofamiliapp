...
-- Versão 2 - Executando automaticamente

-- Limpar dados existentes (opcional)
TRUNCATE TABLE farmers, government_programs, news, market_prices, sales_opportunities, weather_data RESTART IDENTITY CASCADE;
...
SELECT 
    ...
    (SELECT COUNT(*) FROM farmers) as total_agricultores,
    (SELECT COUNT(*) FROM government_programs) as total_programas,
    (SELECT COUNT(*) FROM news) as total_noticias,
    (SELECT COUNT(*) FROM market_prices) as total_precos,
    (SELECT COUNT(*) FROM sales_opportunities) as total_oportunidades,
    (SELECT COUNT(*) FROM weather_data) as total_clima;
